...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/07/22 16:08:25 by ebatchas          #+#    #+#             */
/*   Updated: 2019/07/22 21:37:19 by ebatchas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	ft_point_vector(t_point p, t_vector *r1, t_point p2, t_vector *r2)
{
	r1->x = p.x;
	r1->y = p.y;
	r2->x = p2.x;
...
t_vector	ft_intersect(t_ray ray, t_wall wall, int *status)
{
	...
	*status = 0;
	c.p1 = (t_vector){ray.pos.x, ray.pos.y};
	c.p2 = (t_vector){c.p1.x + ray.dir.x, c.p1.y + ray.dir.y};
  ...
  int *status)
{
	...
	*status = 0;
	c.p1 = (t_vector){player.x, player.y};
	c.p2 = (t_vector){player.x + dir.x, player.y + dir.y};
	ft_point_vector(wall.p1, &c.p3, wall.p2, &c.p4);
	c.den = ((c.p1.x - c.p2.x) * (c.p3.y - c.p4.y) - (c.p1.y - c.p2.y) *
			(c.p3.x - c.p4.x));
	if (c.den == 0)
